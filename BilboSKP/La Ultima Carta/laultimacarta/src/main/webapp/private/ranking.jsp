		...
		<div style="display: flex;padding: 1.25rem;">
			...
			<div class="max-width">
				<div class="page-header">
					...
					<div class="tabs-list">
						<a href="ranking"
							class="${param.tipo != 'clases' ? 'active' : ''}"> <fmt:message
								key="ranking_nacional" />
						</a>
						<c:if test="${usuario != null}">
							<a href="ranking?tipo=clases"
								class="${param.tipo == 'clases' ? 'active' : ''}"> <fmt:message
									key="ranking_clases" />
							</a>
						</c:if>
				...
				<div class="grid">
					...
					<div class="main-content">
						<div class="card">
							<div class="card-header">
								<div class="card-badge">
									<span class="icon icon-trophy"></span> <span> <c:choose>
											<c:when test="${param.tipo == 'clases'}">
												<fmt:message key="clasificacion_clases" />
											</c:when>
											<c:otherwise>
												<fmt:message key="clasificacion_general" />
											</c:otherwise>
										</c:choose>
									</span>
								</div>
								<h2 class="card-title">
									<c:choose>
										<c:when test="${param.tipo == 'clases'}">
											<fmt:message key="top_clases_2025" />
										</c:when>
										<c:otherwise>
								...
								<!-- Show reset button only for class ranking and if user is admin -->
								<c:if
									test="${param.tipo == 'clases' && usuario != null && usuario.rol.id == 1}">
									<div class="action-buttons">
										<a href="reiniciarRanking" class="btn btn-primary"> <fmt:message
												key="reiniciar_ranking" />
										</a>
									</div>
								</c:if>
							</div>
							...
							<div class="card-content">
								<c:choose>
									<c:when test="${param.tipo == 'clases'}">
										...
										<c:choose>
											<c:when test="${empty rankingClases}">
												<div class="text-center">
													<p>
														<fmt:message key="no_datos_ranking" />
													</p>
												</div>
											</c:when>
											<c:otherwise>
												<div class="ranking-list">
													<c:forEach items="${rankingClases}" var="item"
														varStatus="status">
														<div class="ranking-item">
															<div class="rank">${status.index + 1}</div>
															<div class="participant-info">
																<div class="participant-name">${item.nombreClase}</div>
																<div class="participant-location">${item.nombreCentro}</div>
															</div>
															<div class="score">${item.puntuacionTotal}</div>
														</div>
													</c:forEach>
												</div>
											</c:otherwise>
									...
									<c:otherwise>
										<!-- National Ranking Content with Tabs -->
										<div class="tabs">
											<div class="tabs-list">
						...
						<div class="methodology-card">
							...
							<c:choose>
								<c:when test="${param.tipo == 'clases'}">
									<!-- Class Ranking Methodology -->
									<div class="methodology-steps">
										<div class="instruction-step">
											<strong><fmt:message key="aciertos" /> (50%):</strong>
											<fmt:message key="descripcion_aciertos" />
										</div>
										<div class="instruction-step">
											<strong><fmt:message key="tiempo" /> (30%):</strong>
											<fmt:message key="descripcion_tiempo" />
										</div>
										<div class="instruction-step">
											<strong><fmt:message key="pistas" /> (20%):</strong>
											<fmt:message key="descripcion_pistas" />
										</div>
									</div>
									<p class="text-center text-sm text-muted mt-4">
										<fmt:message key="puntuacion_formula" />
										: (aciertos * 10) - (pistas * 2) + bonus_tiempo
									</p>
								</c:when>
								<c:otherwise>
						...
						<c:choose>
							<c:when test="${param.tipo == 'clases'}">
								...
								<div class="card">
									<div class="card-header">
										<div class="card-badge">
											<span class="icon icon-info"></span> <span><fmt:message
													key="informacion_ranking" /></span>
										</div>
										<h3 class="card-title">
											<fmt:message key="como_funciona_ranking" />
										</h3>
									</div>

									<div class="card-content">
										<p>
											<fmt:message key="ranking_descripcion1" />
										</p>
										<p>
											<fmt:message key="ranking_descripcion2" />
										</p>
										<p>
											<fmt:message key="ranking_descripcion3" />
										</p>
									</div>
								</div>
									...
									<div class="card-header">
										<div class="card-badge">
											<span class="icon icon-calendar"></span> <span><fmt:message
													key="reinicio_ranking" /></span>
										</div>
									</div>

									<div class="card-content">
										<p>
											<fmt:message key="reinicio_info1" />
										</p>
										<p>
											<fmt:message key="reinicio_info2" />
										</p>
										<ul>
											<li><fmt:message key="reinicio_automatico" /></li>
											<li><fmt:message key="reinicio_manual" /></li>
										</ul>
									</div>
								</div>
							</c:when>
							<c:otherwise>
								<!-- National Ranking Sidebar -->
								<div class="card">
									<div class="card-header">
										<div class="card-badge">
