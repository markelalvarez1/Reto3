...
public class CuponDAO {
	...
	public List<Cupon> crearCuponesMasivos(int suscripcionId, double precio, int cantidad, Connection conexion) {
		List<Cupon> cuponesCreados = new ArrayList<>();

		for (int i = 0; i < cantidad; i++) {
			Cupon cupon = Cupon.nuevoCupon(suscripcionId);
	...
	public boolean anadirCuponesToSuscripcion(Suscripcion s, Connection conexion) {
		Connection con = conexion;
		PreparedStatement ps = null;
		...
		try {
			String sql = "INSERT INTO cupon (suscripcion_id, fechaCompra, fechaCaducidad, precio, estado) "
					+ "VALUES (?, ?, ?, ?, ?)";
			ps = con.prepareStatement(sql);
			for(Cupon c : s.getCupones()) {
				ps.setInt(1, s.getId());
				ps.setDate(2, Date.valueOf(c.getFechaCompra()));
				ps.setDate(3, Date.valueOf(c.getFechaCaducidad()));
				ps.setDouble(4, c.getPrecio());
				ps.setString(5, c.getEstadoCupon().name());
				ps.addBatch();
			}
